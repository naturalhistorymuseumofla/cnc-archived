<?php
/* 
 *   Template Name: Leaderboard-2020
 */
get_header();
?>
<!-- jQuery -->
...
<div id="leadOverlay">
	...
	<p style="color:#fff; text-align:center; width:100%;">Leaderboard 2020...</p>
</div>
</div>
		...
		<div class="container">
			...
			<div class="row" style="margin-top:50px; margin-bottom:30px;">
				<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12">
					<h1 class="weight-500" style="text-align:center;">2020 LEADERBOARD</h1>
...
$(function(){
	...
	//finalResults(); // This pull from CSV
	testUmbrellaApi(); // This pulls from iNaturalist
		// api1();
...
function testUmbrellaApi(){
	...
	getData('city-nature-challenge-2020').done(function(projects){
		...
		for(var i = 0; i < projects.results.length; i++){
			...
			if(projects.results[i].project.title == 'City Nature Challenge 2020: Prague'){
				console.log('Removed Prague');
			} else {
			...
			if(deferredObjs.length === projects.results.length-1){ 
				...
				.then( (data) => {
					...
					for(i = 0; i < data.length; i++){
						...
						console.log(deferredObjs);
						console.log("end deferredObjs");
					}

					// Get CSV - Daily Uploads from Amy
					$.ajax({
						//url: "../wp-content/uploads/2019/04/leaderboard2019.csv",
						//url: "../wp-content/uploads/2020/01/leaderboard2020.csv",
						url: "..//wp-content/uploads/2020/04/leaderboard2020.csv",
						dataType: "text",
						success:function(data){
							//console.log(data);
							var city_data = data.split("//");
							...
							for(var i = 1; i < city_data.length-1;i++){
								var cell_data = city_data[i].split(",");
									var projObj = { //creates project obj so that table can be sorted.
										'title' : cell_data[0],
										'observations': parseInt(cell_data[1]),
										'species' : parseInt(cell_data[2]),
										'people' : parseInt(cell_data[3])
									};
									deferredObjs.push(projObj);
									//console.log(projObj);
							}

							// At this point, all data is in deferredObjs...
							// Including iNaturalist, Natusfera(Santiago) and CSV...
					...
					// This one works, added after ajax try
					// deferredObjs = deferredObjs.sort(function(a,b){
					// 	return a.observations - b.observations;
					// });
					// deferredObjs.reverse();
