<?php
/* 
 *   Template Name: Collective-2021-Covid
 */
get_header();
?>
<!-- jQuery -->
	...
	<div class="mt-4 table-responsive">
		<table class="table " id ="table-mobile" style="margin-top:100px;">
			<thead style="border-left: none; border-right: none;">
				<tr>
					<th class='srt-btn-header-city srt-btn-header-city-list titleHeader'>City/Country/Region</th>
		...
		<div class="row" style="margin-top:50px;">
			...
			<div class="container mt-4 table-responsive">
				<table class="table " id ="table">
					<thead style="border-left: none; border-right: none;">
						<tr>
							<th class='srt-btn-header-city srt-btn-header-city-list titleHeader'>City/Region/Country</th>
...
function finalResults(){
	$.ajax({
		url: "../wp-content/uploads/2021/05/final_results_2021.json",
		dataType: "json",
		success:function(data){
			...
			for(i=0; i < deferredObjs.length; i++){
				totalObservations = parseInt(totalObservations) + parseInt(deferredObjs[i].observations);  // Converting CSV or TSV to JSON may change pair values to string.  So make all units the same type. 5.11 JC
				totalSpecies = parseInt(totalSpecies) + parseInt(deferredObjs[i].species); //removed to only use totalSpecies 04.09.21
				totalPeople = parseInt(totalPeople) + parseInt(deferredObjs[i].people);
			}

			$(".total-observations").html(1270767);
			$(".total-species").html(45399); // JC Note: The final CSV numbers amy provided still didn't add up to these figures correctly. So i'm manually inputting 5.11.21
			$(".total-people").html(52777);
	...
	*/
	...
	//   console.log("This is at the const taxonStats side", taxonStats)
      ...
      const projObj2 =  {
	    'title': project.title.replace("CNC 2021 ", ""),
	...
	*/
	...
	getData('city-nature-challenge-2021').done(function(projects){
		...
		for(var i = 0; i < projects.results.length; i++){
			// testthisyo is the total species count
			...
			/*********** Removing CNC miscellaneous tags from iNaturalist 04.19.21 *********/
			//attempting to remove all accent/non-standard symbols 
			let moreTitle = fixedTitle.normalize("NFD").replace(/[\u0300-\u036f]/g, "") 
			//Removing non-essential words in City List
			let moreFixed = moreTitle.replace(/[():]|Biomaratón|biomaraton|Biomarató\s|Biomarató|biomarato|\scnc2021|2021|CNC|naturalista|#CNCSOFLO|CityNatureChallenge/gi, '')
			 .replace('City Nature Challenge ','').replace('City Nature Challenge:','').replace('City Nature Challenge-','').replace('City Nature Challenge -','').replace('City Natura Challenge','')
			 .replace('City Natural Challenge','').replace("CNC2021",'').replace('Reto Naturalista','').replace('Biomarató Barcelona (CNC 2021)','Barcelona').replace('Corazón Latino','')
			 .replace('Reto Naturalista Urbano','').replace('Reto Urbano', '').replace('Reto Urbano GAM','').replace('(CNC2021)','').replace('CNC2021','').replace('CNC','').replace(' - ','')
			 .replace("[Bangkok, Thailand]\”", "Bangkok, Thailand").trim();
			var projectObj = {
				'title':  moreFixed,
		...
		*/
		//if(deferredObjs.length === projects.results.length-1){ 
			...
			// Chaining methods to promises so that each fetch call happens after the next sequentially - JC 04.13.21
			}).then(() => earthCall()).then(() => getAmyDataCall()).then(() => getSZCall()).then(()=> {
					...
					function dynamicSort(property) {
						...
						for(i=0; i < deferredObjs.length; i++){
							...
							totalSpecies = totalSpecies //+ deferredObjs[i].species;  Removed to only use iNaturalist total species
							totalPeople = totalPeople + deferredObjs[i].people;
						}
						

						$(".total-observations").html(totalObservations);
						$(".total-species").html(totalSpecies);  //totalSpecies has only iNaturalist
		...
		*/
				...
				// Create new object from JSON data(removing array bracket)
				 let results =  {
							"title":  json[0].title ? json[0].title : "n/a",
		...
		*/
				...
				async function getAmyDataCall(){
					...
					if (isCSVCheck(amyURL)) {
						...
						response.map(json => {
							let results =  {
							"title":  json.title ? json.title : "n/a",
